...
- `refine_on_verify`「验证时精炼」真正生效：验证发现内容被修正时，先用 LLM 将修正内容整理为精炼知识卡（保持事实、≤200 字）再入库，失败自动降级；批量验证同路径生效。

### 变更

- 页面文案清理：移除 `_conf_schema.json`、`priority_topics`、`data/logs/` 路径、并发数等实现细节与重复说明；诊断弹窗「关心领域」配置值与运行值相同时不再重复列出；补齐排序胶囊和分页信息断行兜底。
- 标签页与按钮加载态收编至 `SeriesUI.bindTabs` / `SeriesUI.setBusy`，转义统一使用 `SeriesUI.escapeHtml`，确认框改为惰性获取以恢复 DOM 回退。
- 删除约 236 行无引用的旧表格样式，导入流程抽出公共函数。

## 1.8.6 - 2026-09-16

### 变更

- 支持核接管「一键读取当前配置 / 一键固化到插件」：一键读取/固化：snapshot 暴露 native_value；固化写入 active_learner_settings.json（先备份、原子落盘、失败回滚、运行时同步）。

## 1.8.5 - 2026-09-14

### 变更

- 全系列统一滑块开关：设置中心 13 个布尔开关（精炼 / 自主学习 / 联网搜索 / B 站来源 / 跨领域回复 / 内置知识库精炼 / URL 来源启停 / 日志自动滚动等）改用 series.ui 共享开关（`si-toggle`），删除页面自绘伪开关样式。
- 导入列表逐条勾选、学习主题与知识库文档勾选保持原生复选框——列表选择语义不变。
...
- 记忆列表改为两行式行卡：标题 / 预览 / 元信息三段，状态与置信度右侧成组，行内操作收进 kebab 菜单。
- 顶部动作收敛为「导入 / 主动学习 / 更多」，概览统计、搜索与作用域合并为一条摘要带；移动端每页 10 条。
- 诊断快照窗口修复：积压时按最早窗口返回并带 `has_more`，消费端可追平，不再静默丢事件。

## 1.8.2 - 2026-09-13

### 变更

- 记忆页补齐长 ID 短显示与一键复制（作用域、自然人），修复作用域筛选空值导致下拉空白的问题。
- 记忆来源值改为中文映射（主动学习/搜索学习/交叉验证），未知来源不再回显内部键名。
- 移动端记忆卡内边距统一、行高一致；页头去掉双层外框。
- 同步 series.ui 1.0.1：通用控件选择器降权，页面自身样式不再被静默覆盖。

## 1.8.1 - 2026-09-13

### 变更

- 优化知页面任务优先布局：记忆列表前置、移动端每页 10 条、日志和验证详情下沉、高级配置保存栏 sticky；补齐共享 UI 布局组件。

## 1.8.0 - 2026-09-13

### 新增

- 全面接入 `series.ui@1.0` Glass Aurora 组件库，统一页面背景、卡片、表单、按钮、表格、弹窗和状态反馈；保留知识管理专属布局并改善移动端表格。
...
- 接入 `series.module@1.0` 与 `series.webui@2.0`：核统一接管时可查看知识概览、记忆列表，并在受控 Scope 内导入 JSON/TXT/MD、导出短时制品文件。
- 会话期图记忆选路接入统一 `fast` 模型角色；学习、验证、精炼与主题扩展使用 `reasoning` 角色。

### 修复

- 修正模块契约中误写的 JSON 布尔字面量，避免调用 `series_module_contract` 时触发 `NameError`。
- 修正检索过滤回归用例的 Scope 隔离，完整测试恢复为绿色。

## 1.5.4 - 2026-08-26

### 性能

- 记忆召回按需门控：没有回忆/回忆指代时跳过全库 FTS 与向量检索，减少无关请求开销。

- 检索与嵌入结果增加 TTL 缓存，并补充 series diagnostics 观测；缓存失效或契约不可用时仍回退到原有查询路径。

## 1.5.3 - 2026-08-21

- 接入 `series.control@1.0`，支持知的安全运行字段统一接管与 native 回退。

- LLM Provider 解析在本插件显式配置不可用时，兼容 `series.model_router@1.0` 的“核”统一对话模型路由；契约缺失或不可用时继续使用 AstrBot 原生回退。
...
- **群黑话（四期功能汇总）**
  - 被动捕获：在 `on_llm_request` 中用 regex 提取候选黑话词（不调 LLM），说话人去重 + 防刷屏计数时间窗，候选攒够后合并为 1 次批量 LLM 学习
  - 确定性召回：已学词条子串匹配命中后注入释义，群义优先于通用梗，统一带「群黑话/通用梗 · 仅供参考」标签
  - 跨群晋升：同一词条在足够多的群学到且 LLM 判定为通用梗时自动晋升 global（唯一自动写 global 的通道）；支持管理员手动晋升，重复晋升合并历史留痕
  - 管理端审核：Dashboard 新增「🏷 黑话」页签（候选队列 / 已学词条 / 拉黑列表），支持晋升全局、拒绝候选、拉黑 / 解除拉黑；拉黑名单落 `slang_blocklist` 表（schema v5），捕获路径逐词过滤，60s 实例级缓存避免重复查库
  - 新增 `lookup_slang` LLM 工具：查询黑话/梗释义（当前会话 scope + global 兜底），未命中时引导群友解释、插件被动学习

## 1.5.1 - 2026-08-20

### 修复

- 统一诊断日志不再只显示“详细信息在知的独立日志页查看”的占位摘要；现在保留模块、函数、行号、异常类型和脱敏后的日志详情，核的统一日志页可直接展开排查。

## 1.5.0 - 2026-08-19

### 变更

- 发布管理页可访问性、Bridge 恢复与动效优化。

### 管理页可访问性与恢复

- 管理页设置与记忆导入标签页补齐 `aria-controls`、`aria-labelledby`、键盘焦点和隐藏状态同步。
- Bridge 初始化失败时保留页面控件可用性，并提供受限动效与触屏 hover 降级。

### 新增

- 新增默认关闭的关联图补充检索：从既有 `memories` 派生 Cue-Tag 索引，在普通召回不足的比较、因果、时间线和依赖问题中执行最多两跳的受限重建。
- 新增 `off`、`fast`、`smart` 三种重建模式；`fast` 只访问本地 SQLite，`smart` 最多增加一次 2.5 秒的候选排序调用，超时或异常时自动退回本地结果。

### 安全与兼容

- 图索引不是独立事实库，返回正文仍来自原记忆；检索继续遵守私聊、群聊和可选全局知识的作用域边界。功能默认关闭，旧配置和原聊天行为保持不变。
- 数据库 schema 升至 v3，首次打开会从现有 topic、content 和 keywords 确定性回填索引，不调用 LLM；写入、更新和删除会同步图边。
- 系列诊断契约补齐 `series_id`、插件身份和统一的读取/清空能力声明，可由“核”动态发现。
...
- 补充索引迁移、去重预算、作用域隔离、更新删除同步、复杂问题门控和智能选路禁止虚构 ID 的回归测试。

## 1.4.1 - 2026-08-03

### 变更

- 优化管理页日志：内容未变化时不重绘整表，切换作用域时减少无关请求，批量删除改为有限并发并显示处理中状态。
- 统一弹窗确认、Escape 关闭最上层弹窗、空状态和无结果提示，并补充键盘焦点与 ARIA 语义。
- 配置说明改为中文大白话，保留手动 URL、MediaWiki 和来源优先级等现有功能。
