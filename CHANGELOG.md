...
- **LLM 将记忆参考输出到回复中**：记忆注入时 `📚 参考资料:` 被放入 LLM 可见上下文，LLM 直接原样输出。修复分两步：(1) 从 `on_llm_request` 注入内容中移除 `refs_lines`，改为写入 info 日志；(2) `on_llm_response` 改为从 LLM 原始回复中用正则删除 `📚 参考资料:` 相关行，不再通过 `extra_assistant_content_parts` 追加
