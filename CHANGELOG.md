...
- 记忆注入条目携带写入时间（规范 3.7「注入即标注」）：`【内部知识】` 前缀新增「时间：YYYY-MM-DD HH:MM」字段（取 `created_at`，缺失回退 `updated_at`，均无则标注「时间：未知」），模型据此判断知识时效，不再凭直觉假设记忆新旧；时间字段作为结构元数据不占用注入字符预算，预算口径与此前完全一致。

## 1.5.4 - 2026-08-26

- 检索与嵌入新增 TTL 缓存，补充 series diagnostics 接线。
- 记忆召回按需门控：无回忆指代时跳过全库 FTS/向量检索。
